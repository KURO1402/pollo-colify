...
const FormularioMedioPago = ({ medioPago = null, onSubmit, onCancelar, cargando }) => {
  const [nombre, setNombre] = useState("");

  useEffect(() => {
    setNombre(medioPago?.nombre_medio_pago ?? "");
  }, [medioPago]);

  const handleSubmit = (e) => {
    e.preventDefault();
    if (!nombre.trim()) return;
    onSubmit({ nombreMedioPago: nombre.trim() });
  };
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      <div>
        ...
        <input
          ...
          placeholder="Ej: Efectivo, Yape, Tarjeta..."
      ...
      <div className="flex justify-end gap-3">
        ...
        >
          {cargando ? "Guardando..." : medioPago ? "Actualizar" : "Crear"}
        </button>
      </div>
    </form>
  );
...
export default FormularioMedioPago;